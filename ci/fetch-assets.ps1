...
$downloads = @{
    ...
    "51Degrees.csv" = {
        ./steps/fetch-csv-assets.ps1 -RepoName $RepoName -LicenseKey $DeviceDetection -Url $DeviceDetectionUrl
        Move-Item -Path $RepoName/51Degrees-TacV3.4.trie.zip -Destination 51Degrees-TacV3.4.trie.zip
        Expand-Archive -Path 51Degrees-TacV3.4.trie.zip
        Get-Content -TotalCount 1 51Degrees-TacV3.4.trie/51Degrees-Tac-All.csv | Out-File $assets/$file # We only need a header
        Remove-Item -Path 51Degrees-TacV3.4.trie.zip, 51Degrees-TacV3.4.trie/51Degrees-Tac-All.csv
    }
}
